...
plt.tight_layout()
plt.savefig('time_series/time_series_predictions.png')
plt.close()
...
plt.tight_layout()
plt.savefig('time_series/seasonal_patterns.png')
plt.close()

# Separate Plot (Monthly Only)
plt.figure(figsize=(15, 5))

# Monthly seasonality for the separate plot
monthly_avg_separate = df.groupby(df['InvoiceDate'].dt.month)['TotalValue'].mean()
monthly_avg_separate.plot(kind='bar')
plt.title('Average Sales by Month')
plt.xlabel('Month')
plt.ylabel('Average Sales')

plt.tight_layout()
plt.savefig('time_series/monthly_sales_only.png')
plt.close()